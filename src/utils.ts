...
import { SampleMaterial } from "./assets/materials/sample-material";
...
export class GameUtils {
  ...
  public static createGround(scene: BABYLON.Scene): BABYLON.Mesh {
    ...
    // let material = new SampleMaterial("material", scene);
  ...
  public static createWater(scene: BABYLON.Scene): BABYLON.WaterMaterial {
    ...
    let waterMaterial = GameUtils.createWaterMaterial(
      "water",
      "./assets/texture/waterbump.png",
      scene
    );
  ...
  public static createGui(
    btnText: string,
    btnClicked: (button: GUI.Button) => void
  ) {
   ...
   */
  public static createMeshFromObjFile(
    folderName: string,
    fileName: string,
    scene: BABYLON.Scene,
    ...
    if (!fileName) {
      return Observable.throw(
        "GameUtils.createMeshFromObjFile: parameter fileName is empty"
      );
    }
    if (!scene) {
      return Observable.throw(
   ...
   * @param scene
   */
  public static createShark(
    scene: BABYLON.Scene
  ): Observable<BABYLON.AbstractMesh> {
    // create a mesh object with loaded from file
    let rootMesh = BABYLON.MeshBuilder.CreateBox(
      "rootMesh",
      { size: 1 },
      scene
    );
    ...
    return new Observable(observer => {
      GameUtils.createMeshFromObjFile(
        "mesh/",
        "mesh.obj",
        scene,
